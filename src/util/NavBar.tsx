...
import CreatorActions from '@/components/CreatorDashboard/CreatorActions'

const NavBar = ({ showSearch = true, tagline = "Let's play", dashboard = false }) => {
  return (
    <nav className='fixed top-0 w-full h-20 z-[50] bg-[#252731] px-2 lg:px-4 flex items-center justify-between shadow-sm'>
      ...
      {dashboard ? <CreatorActions /> : <Actions />}
    </nav>
  )
}
